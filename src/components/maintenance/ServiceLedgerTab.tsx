...
export default function ServiceLedgerTab({
  readOnly, performedBy, records, onAddRecord, onUpdateRecord, onDeleteRecord,
                ...
                filteredRecords.map((r) => {
                  ...
                  return (
                    ...
                    <tr className="hover:bg-slate-50/50 transition-colors align-top">
                      ...
                      <td className="px-3 py-2.5 whitespace-nowrap">
                        {readOnly ? (
                          <span className="text-slate-300">{r.invoiceNumber || '-'}</span>
                        ) : (
              ...
              <form id="maintenance-entry-form" onSubmit={handleSubmit} className="space-y-3.5" autoComplete="off">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <DateInput required value={date} onChange={(e) => setDate(e.target.value)} max={new Date().toISOString().slice(0, 10)} className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 text-slate-800 font-mono" />
                  </div>
                  <div>
