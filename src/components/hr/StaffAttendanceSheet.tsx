    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 space-y-4">
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          <button onClick={autoFillSundaysAndHolidays} className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 rounded-lg font-semibold cursor-pointer">
            ...
            <thead>
              <tr className="bg-slate-50 sticky top-0">
                ...
                {Array.from({ length: totalDays }, (_, i) => i + 1).map(day => (
                  <th key={day} className="px-1 py-2 text-center font-bold text-slate-400 w-9">{dayLabel(month, day)}</th>
                ))}
              </tr>
            </thead>
      ...
      {popover && createPortal(
        <>
          ...
          <div style={{ position: 'fixed', top: popover.top, left: popover.left }} className="z-[100] bg-white border border-slate-200 rounded-lg shadow-xl p-3 w-56 text-xs space-y-2">
            ...
            <button onClick={savePopover} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-1.5 rounded-lg uppercase text-[10px] cursor-pointer">Save</button>
          </div>
        </>,
        document.body
      )}
