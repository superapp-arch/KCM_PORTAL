// Shared Petty Cash Location canonicalization (2026-09-18 direct request) -
// the same real place kept getting logged under several different
// spellings/casings depending on who typed it freehand (e.g.
// "BANGALURE"/"bangalure"/"Bangalure" all meant to be one place), which
// fragmented the ledger's own Location column/filter into near-duplicate
// values instead of one clean one. Used by both PettyCash.tsx (autocomplete
// suggestions + auto-correct on blur, for new entries going forward) and
// src/db/service.ts's normalizePettyCashLocationNames migration (one-time
// cleanup of already-saved rows) - kept in one place so the two can never
// drift on what counts as "the same place."
//
